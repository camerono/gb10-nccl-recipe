...
what it takes to get nccl past 12 gbps on a pair of nvidia gb10 boxes (dgx spark, asus ascent gx10, msi edgexpert — same soc, same kerfuffle, same fix) connected by a single qsfp56 cable. the official playbook gets you halfway. this gets you the rest, eh.
...
2. **list both pcie-domain halves of the cabled port + `NCCL_IB_MERGE_NICS=1`.** the cx-7 nic exposes each physical qsfp port as two rdma hcas, one per pcie domain. one cable, two devices. miss this and you cap at half line-rate, like trying to play *paranoid* with one guitar string. i could not find this in any official nvidia doc.
...
if you're staring at `GDR 0` and trying to flip it: stop. it's not flipping. (i tried. for the better part of a day. as God is my witness, i thought GDR could fly.)
